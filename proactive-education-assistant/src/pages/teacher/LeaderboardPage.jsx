    ...
    <div className="px-4 py-6 bg-gradient-to-br from-slate-50 to-slate-100 min-h-screen" style={{ paddingTop: '5rem' }}>
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="text-center md:text-left">
          <h1 className="text-3xl font-bold text-slate-900">🏅 {t("teacher_leaderboard.title", "Leaderboard")}</h1>
          <p className="text-slate-600 mt-1">
            {t("teacher_leaderboard.subtitle", "See how you stack up against other teachers in your school and beyond")}
          </p>
        </div>
        ...
        {teacherPosition && (
          <div className="bg-gradient-to-r from-blue-500 to-blue-600 rounded-xl p-6 shadow-lg text-white">
            <div className="grid md:grid-cols-4 gap-4 items-center">
              <div>
                <p className="text-sm font-medium text-blue-100">{t("teacher_leaderboard.current_rank", "Your Current Rank")}</p>
              ...
              <div>
                <p className="text-sm font-medium text-blue-100">{t("teacher_leaderboard.total_xp", "Total XP")}</p>
                <p className="text-3xl font-bold">{teacherPosition.totalXP.toLocaleString()}</p>
              </div>
              <div>
                <p className="text-sm font-medium text-blue-100">{t("teacher_leaderboard.level", "Level")}</p>
                <p className="text-3xl font-bold">{teacherPosition.level}</p>
              </div>
              <div>
                <p className="text-sm font-medium text-blue-100">{t("teacher_leaderboard.badges", "Badges")}</p>
        ...
        {topThree.length > 0 && (
          <div className="space-y-4">
            <h2 className="font-semibold text-slate-900 text-lg flex items-center gap-2">
              <span>🏆</span> {t("teacher_leaderboard.top_performers", "Top Performers")}
            </h2>
        ...
        <div className="space-y-4">
          <h2 className="font-semibold text-slate-900 text-lg flex items-center gap-2">
              <span>📊</span> {t("teacher_leaderboard.full_rankings", "Full Rankings")}
          </h2>
          ...
          {loading ? (
            <div className="bg-white rounded-xl p-8 text-center">
              ...
              <p className="text-slate-600 mt-4">{t("teacher_leaderboard.loading", "Loading leaderboard...")}</p>
            </div>
          ) : (
            <LeaderboardTable
        ...
        <div className="bg-gradient-to-r from-purple-50 to-purple-100 rounded-xl p-6 border border-purple-200">
          <h3 className="font-semibold text-slate-900 mb-3 flex items-center gap-2">
            <span>📈</span> {t("teacher_leaderboard.stats", "Leaderboard Stats")}
          </h3>
          <div className="grid md:grid-cols-3 gap-4 text-sm">
            <div>
              <p className="text-slate-600">{t("teacher_leaderboard.total_teachers", "Total Teachers")}</p>
              <p className="text-2xl font-bold text-purple-700 mt-1">{leaderboard.length}</p>
            </div>
            <div>
              <p className="text-slate-600">{t("teacher_leaderboard.average_level", "Average Level")}</p>
            ...
            <div>
              <p className="text-slate-600">{t("teacher_leaderboard.highest_xp", "Highest XP")}</p>
              <p className="text-2xl font-bold text-purple-700 mt-1">
