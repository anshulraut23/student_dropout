...
export default function ScoreHistoryPage() {
  ...
  const displayStatusLabel = (status) => {
    if (status === 'submitted' || status === 'verified') return 'present';
  ...
  const loadScoreHistory = async () => {
    ...
    try {
          ...
          for (const exam of exams) {
            try {
              ...
              if (marksResponse.success && marksResponse.marks) {
                ...
                const presentMarks = marks.filter(m => isPresentStatus(m.status));
      ...
      if (allHistory.length === 0) {
        setMessage({ type: "info", text: t("teacher_score.no_records_found", "No score records found.") });
        setTimeout(() => setMessage({ type: "", text: "" }), 3000);
      }
    } catch (error) {
      console.error('Failed to load score history:', error);
      setMessage({ type: "error", text: t("teacher_score.failed_to_load", "Failed to load score history.") });
    } finally {
      setLoading(false);
    }
  };
  ...
  const exportToExcel = () => {
    try {
      ...
      setMessage({ type: "success", text: t("teacher_score.export_success", "✓ Exported successfully!") });
      setTimeout(() => setMessage({ type: "", text: "" }), 2000);
    } catch (error) {
      setMessage({ type: "error", text: t("teacher_score.export_failed", "Failed to export") });
    }
  };
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="mb-8">
              <h1 className="text-3xl md:text-4xl font-bold mb-2" style={{ fontFamily: 'var(--font-heading)', color: 'var(--text-dark)' }}>
                {t("teacher_score.title", "Score History")}
              </h1>
              <p className="text-sm md:text-base" style={{ color: 'var(--gray)' }}>
                {t("teacher_score.subtitle", "View exam scores and performance analytics")}
              </p>
            </div>
            ...
            <div className="mb-8 p-6 rounded-lg border" style={{ background: 'var(--white)', borderColor: 'rgba(26, 111, 181, 0.12)' }}>
              <div className="flex items-center gap-2 mb-5">
                ...
                <h2 className="text-sm font-semibold" style={{ color: 'var(--text-dark)' }}>{t("teacher_score.filters", "Filters")}</h2>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-5">
                ...
                <div>
                  <label className="block text-xs font-semibold mb-2" style={{ color: 'var(--text-dark)' }}>{t("teacher_score.class", "Class")}</label>
                  <select
                    name="classId"
                    ...
                    className="w-full px-4 py-2.5 text-sm horizon-input"
                  >
                    <option value="">{t("teacher_score.all_classes", "All Classes")}</option>
                    {classes.map(cls => (
                ...
                <div>
                  <label className="block text-xs font-semibold mb-2" style={{ color: 'var(--text-dark)' }}>{t("teacher_score.subject", "Subject")}</label>
                  <select
                    name="subjectId"
                  ...
                  >
                    <option value="">{t("teacher_score.all_subjects", "All Subjects")}</option>
                    {subjects.map(s => (
                ...
                <div>
                  <label className="block text-xs font-semibold mb-2" style={{ color: 'var(--text-dark)' }}>{t("teacher_score.from", "From")}</label>
                  <input
                    type="date"
                    name="startDate"
                ...
                <div>
                  <label className="block text-xs font-semibold mb-2" style={{ color: 'var(--text-dark)' }}>{t("teacher_score.to", "To")}</label>
                  <input
                    type="date"
                    name="endDate"
              ...
              <div className="flex justify-end">
                <button onClick={clearFilters} className="horizon-btn-secondary px-6 py-2 text-sm">
                  {t("teacher_score.clear_filters", "Clear Filters")}
                </button>
              </div>
            </div>
            ...
            <div className="rounded-lg overflow-hidden border" style={{ background: 'var(--white)', borderColor: 'rgba(26, 111, 181, 0.12)' }}>
              {loading ? (
                ...
                <div className="text-center py-16">
                  ...
                  <p className="text-sm" style={{ color: 'var(--gray)' }}>{t("teacher_score.no_records", "No records found")}</p>
                </div>
              ) : (
                <>
                  <div className="overflow-x-auto">
                    <table className="w-full">
                      <thead style={{ background: 'var(--light-bg)', borderBottom: '1px solid rgba(26, 111, 181, 0.08)' }}>
                        <tr>
                          <th className="text-left px-6 py-4 text-xs font-semibold uppercase" style={{ color: 'var(--gray)' }}>{t("teacher_score.exam", "Exam")}</th>
                          <th className="text-left px-6 py-4 text-xs font-semibold uppercase" style={{ color: 'var(--gray)' }}>{t("teacher_score.date", "Date")}</th>
                          <th className="text-left px-6 py-4 text-xs font-semibold uppercase" style={{ color: 'var(--gray)' }}>{t("teacher_score.class", "Class")}</th>
                          <th className="text-left px-6 py-4 text-xs font-semibold uppercase" style={{ color: 'var(--gray)' }}>{t("teacher_score.subject", "Subject")}</th>
                          <th className="text-center px-6 py-4 text-xs font-semibold uppercase" style={{ color: 'var(--gray)' }}>{t("teacher_score.students", "Students")}</th>
                          <th className="text-center px-6 py-4 text-xs font-semibold uppercase" style={{ color: 'var(--gray)' }}>{t("teacher_score.avg_percent", "Avg %")}</th>
                          <th className="text-center px-6 py-4 text-xs font-semibold uppercase" style={{ color: 'var(--gray)' }}>{t("teacher_score.pass_percent", "Pass %")}</th>
                          <th className="text-center px-6 py-4 text-xs font-semibold uppercase" style={{ color: 'var(--gray)' }}>{t("teacher_score.actions", "Actions")}</th>
                        </tr>
                      </thead>
                      <tbody>
                  ...
                  <div className="px-6 py-3 text-xs" style={{ background: 'var(--light-bg)', borderTop: '1px solid rgba(26, 111, 181, 0.08)', color: 'var(--gray)' }}>
                    {t("teacher_score.showing_records", "Showing {{count}} record(s)", { count: filteredHistory.length })}
                  </div>
                </>
              )}
            </div>
            ...
            {showDetailsModal && selectedRecord && (
              <>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
                  <div 
                    className="bg-white rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto pointer-events-auto modal-content"
                  ...
                  >
                    ...
                    <div className="sticky top-0 bg-white border-b p-6" style={{ borderColor: 'rgba(26, 111, 181, 0.12)' }}>
                      <div className="flex items-center justify-between">
                        <h3 className="text-xl font-semibold" style={{ fontFamily: 'var(--font-heading)', color: 'var(--text-dark)' }}>
                          {t("teacher_score.score_details", "Score Details")}
                        </h3>
                        <button 
                          onClick={closeModal}
                    ...
                    <div className="p-6 space-y-6">
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 rounded-lg" style={{ background: 'var(--light-bg)' }}>
                        <div>
                          <p className="text-xs" style={{ color: 'var(--gray)' }}>{t("teacher_score.exam", "Exam")}</p>
                          <p className="text-sm font-semibold mt-2" style={{ color: 'var(--text-dark)' }}>{selectedRecord.examName}</p>
                        </div>
                        <div>
                          <p className="text-xs" style={{ color: 'var(--gray)' }}>{t("teacher_score.class", "Class")}</p>
                          <p className="text-sm font-semibold mt-2" style={{ color: 'var(--text-dark)' }}>{selectedRecord.className}</p>
                        </div>
                        <div>
                          <p className="text-xs" style={{ color: 'var(--gray)' }}>{t("teacher_score.subject", "Subject")}</p>
                          <p className="text-sm font-semibold mt-2" style={{ color: 'var(--text-dark)' }}>{selectedRecord.subjectName}</p>
                        </div>
                        <div>
                          <p className="text-xs" style={{ color: 'var(--gray)' }}>{t("teacher_score.date", "Date")}</p>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div className="p-4 rounded-lg" style={{ background: 'rgba(26, 111, 181, 0.08)', borderLeft: '4px solid var(--primary-blue)' }}>
                          <p className="text-xs" style={{ color: 'var(--gray)' }}>{t("teacher_score.students", "Students")}</p>
                        ...
                        <div className="p-4 rounded-lg" style={{ background: 'rgba(34, 197, 94, 0.08)', borderLeft: '4px solid rgb(34, 197, 94)' }}>
                          <p className="text-xs" style={{ color: 'var(--gray)' }}>{t("teacher_score.appeared", "Appeared")}</p>
                        ...
                        <div className="p-4 rounded-lg" style={{ background: 'rgba(168, 85, 247, 0.08)', borderLeft: '4px solid rgb(168, 85, 247)' }}>
                          <p className="text-xs" style={{ color: 'var(--gray)' }}>{t("teacher_score.avg_marks", "Avg Marks")}</p>
                        ...
                        <div className="p-4 rounded-lg" style={{ background: 'rgba(245, 158, 11, 0.08)', borderLeft: '4px solid rgb(245, 158, 11)' }}>
                          <p className="text-xs" style={{ color: 'var(--gray)' }}>{t("teacher_score.pass_rate", "Pass Rate")}</p>
                      ...
                      {selectedRecord.marks && selectedRecord.marks.length > 0 && (
                        <div>
                          <h4 className="text-sm font-semibold mb-4" style={{ color: 'var(--text-dark)' }}>{t("teacher_score.student_scores", "Student Scores")}</h4>
                              ...
                              <table className="w-full text-sm">
                                <thead style={{ background: 'var(--light-bg)', borderBottom: '1px solid rgba(26, 111, 181, 0.08)' }}>
                                  <tr>
                                    ...
                                    <th className="text-left px-4 py-3 text-xs font-semibold" style={{ color: 'var(--gray)' }}>{t("teacher_score.student", "Student")}</th>
                                    <th className="text-center px-4 py-3 text-xs font-semibold" style={{ color: 'var(--gray)' }}>{t("teacher_score.marks", "Marks")}</th>
                                    <th className="text-center px-4 py-3 text-xs font-semibold" style={{ color: 'var(--gray)' }}>%</th>
                                    <th className="text-center px-4 py-3 text-xs font-semibold" style={{ color: 'var(--gray)' }}>{t("teacher_score.grade", "Grade")}</th>
                                    <th className="text-center px-4 py-3 text-xs font-semibold" style={{ color: 'var(--gray)' }}>{t("teacher_score.status", "Status")}</th>
                                  </tr>
                                </thead>
                                <tbody>
                                  {selectedRecord.marks
                                    ...
                                    <tr key={idx} style={{ borderBottom: '1px solid rgba(26, 111, 181, 0.06)' }}>
                                      ...
                                      <td className="px-4 py-3 text-center" style={{ color: 'var(--text-dark)' }}>
                                        {isPresentStatus(mark.status) ? `${mark.marksObtained}/${selectedRecord.totalMarks}` : '-'}
                                      </td>
                                      <td className="px-4 py-3 text-center" style={{ color: 'var(--text-dark)' }}>
                                        {isPresentStatus(mark.status) ? `${mark.percentage}%` : '-'}
                                      </td>
                                      <td className="px-4 py-3 text-center" style={{ color: 'var(--text-dark)' }}>
                                        {mark.grade || '-'}
                                      </td>
                                      <td className="px-4 py-3 text-center">
                                        <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
                                          ...
                                          {isPresentStatus(mark.status)
                                            ? mark.marksObtained >= selectedRecord.passingMarks ? t("teacher_score.pass", "Pass") : t("teacher_score.fail", "Fail")
                                            : displayStatusLabel(mark.status)}
                                        </span>
                                      </td>
                                    </tr>
                                  ))}
