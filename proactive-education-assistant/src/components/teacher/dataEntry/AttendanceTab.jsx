...
export default function AttendanceTab() {
  ...
  const checkExistingAttendance = async () => {
    try {
      ...
      const hasAttendance = result.success && result.marked > 0 && result.attendance && Array.isArray(result.attendance);
  ...
  const loadClasses = async () => {
    try {
      const result = await apiService.getMyClasses();
      if (result.success) {
        const attendanceClasses = (result.classes || []).filter(cls => 
          cls.isIncharge || cls.role === 'incharge' || cls.role === 'both' ||
          cls.role === 'subject_teacher' || (cls.subjects && cls.subjects.length > 0)
        );
  ...
  const downloadTemplate = () => {
    try {
      ...
      worksheet['!cols'] = [{ wch: 25 }, { wch: 15 }, { wch: 12 }, { wch: 10 }];
  ...
  return (
    <>
      ...
      <div className="flex gap-3 mb-6">
        <button
          onClick={() => setMode("manual")}
          className={`mode-button px-4 py-2.5 text-sm font-semibold rounded-lg transition-all ${
            mode === "manual"
              ? "horizon-btn-primary"
              : "horizon-btn-secondary"
          }`}
      ...
      {mode === "manual" ? (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            ...
            <div>
              <label className="block text-sm font-semibold mb-2" style={{ color: 'var(--text-dark)' }}>
                Date <span style={{ color: 'var(--accent-gold)' }}>*</span>
              </label>
              <input
                type="date"
                value={selectedDate}
            ...
            {(attendanceMode === 'subject' || attendanceMode === 'subject-wise' || attendanceMode === 'subject_wise') && availableSubjects.length > 0 && (
              <div>
                <label className="block text-sm font-semibold mb-2" style={{ color: 'var(--text-dark)' }}>
                  Select Subject <span style={{ color: 'var(--accent-gold)' }}>*</span>
                </label>
                <select
                  value={selectedSubject}
                  onChange={(e) => setSelectedSubject(e.target.value)}
          ...
          {/* Info Banner */}
          {(attendanceMode === 'subject' || attendanceMode === 'subject-wise' || attendanceMode === 'subject_wise') && selectedClass && (
            <div className="mb-6 p-4 rounded-lg border-l-4" style={{ background: 'rgba(26, 111, 181, 0.04)', borderLeftColor: 'var(--primary-blue)', borderColor: 'rgba(26, 111, 181, 0.12)' }}>
              <p className="text-sm" style={{ color: 'var(--text-dark)' }}>
                <span className="font-semibold">Subject-wise Attendance:</span> This class uses subject-wise marking.
                {teacherRole === 'subject_teacher' 
                  ? ' You can mark only your assigned subjects.'
                  : ' You can mark all subjects as class incharge.'}
              </p>
            </div>
          )}
            ...
            <div className="mb-6 p-4 rounded-lg border-l-4" style={{ background: 'rgba(34, 197, 94, 0.08)', borderLeftColor: '#16a34a', borderColor: 'rgba(34, 197, 94, 0.3)' }}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-sm font-semibold" style={{ color: '#15803d' }}>Attendance Already Marked</h3>
                    <p className="text-xs mt-1" style={{ color: '#166534' }}>
                      Marked for {new Date(selectedDate).toLocaleDateString()}.
                      <br />
                      <span className="font-medium">
                        Present: {existingAttendance.filter(a => a.status === 'present').length} | 
                        Absent: {existingAttendance.filter(a => a.status === 'absent').length} | 
                  ...
                  <table className="w-full">
                    <thead style={{ background: 'var(--light-bg)', borderBottom: '1px solid rgba(26, 111, 181, 0.08)' }}>
                      <tr>
                        ...
                        <th className="text-left px-4 py-3 text-xs font-semibold uppercase" style={{ color: 'var(--gray)' }}>Enrollment</th>
                        <th className="text-center px-4 py-3 text-xs font-semibold uppercase" style={{ color: 'var(--gray)' }}>Status</th>
                      </tr>
                    </thead>
                          ...
                          <td className="px-4 py-3 text-center">
                            <div className="flex justify-center gap-2">
                              <button
                                ...
                                style={{
                                  background: attendance[student.id] === "present" ? 'var(--primary-blue)' : 'rgba(26, 111, 181, 0.08)'
                                }}
                              >
                                <FaCheck className="inline mr-1" />
                                Present
                              ...
                              <button
                                ...
                                style={{
                                  background: attendance[student.id] === "absent" ? '#dc2626' : 'rgba(220, 38, 38, 0.08)'
                                }}
                              >
                                <FaTimes className="inline mr-1" />
                                Absent
        ...
        <>
          ...
          {selectedClass && students.length > 0 && (
            <div className="mb-6 p-4 rounded-lg" style={{ background: 'rgba(26, 111, 181, 0.04)', border: '1px solid rgba(26, 111, 181, 0.12)' }}>
              <button
                onClick={downloadTemplate}
                className="horizon-btn-secondary px-4 py-2 text-sm inline-flex items-center gap-2"
              >
                <FaDownload />
                Download Template
              </button>
              <p className="text-xs mt-2" style={{ color: 'var(--gray)' }}>
                Download with all students from this class
              </p>
            </div>
          )}
          ...
          {/* Upload Area */}
          <div className="mb-6 border-2 border-dashed rounded-lg p-8 text-center transition-all" style={{ borderColor: 'rgba(26, 111, 181, 0.3)', background: 'rgba(26, 111, 181, 0.02)' }}>
            <FaUpload className="mx-auto text-3xl mb-3" style={{ color: 'var(--primary-blue)', opacity: 0.6 }} />
            <h3 className="text-sm font-semibold mb-1" style={{ color: 'var(--text-dark)' }}>Upload Attendance Data</h3>
            <p className="text-xs mb-4" style={{ color: 'var(--gray)' }}>CSV or Excel file</p>
            
            <label className="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-semibold text-white rounded-lg cursor-pointer transition-all hover:-translate-y-0.5" style={{ background: 'var(--primary-blue)' }}>
              <FaUpload className="text-xs" />
              Choose File
              <input
                type="file"
            ...
            {bulkFile && (
              <div className="mt-4 text-sm" style={{ color: 'var(--gray)' }}>
                Selected: <span className="font-semibold" style={{ color: 'var(--text-dark)' }}>{bulkFile.name}</span>
              </div>
            )}
          </div>
